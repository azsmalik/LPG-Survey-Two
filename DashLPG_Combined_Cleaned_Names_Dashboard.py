# ============================================================
# LPG Country Demand Survey Dashboard - Combined Cleaned Dataset
# Country-level Streamlit dashboard for Pakistan LPG survey analysis using the combined standardized LPG survey response file.
#
# Run locally:
...
#   C:\Ahmed Zahid Malik\Python\LPG Survey Dashboard\LPG_Survey_Combined_Cleaned_Standardized_Names.csv
#
# Deployment note:
#   For Streamlit Cloud, keep LPG_Survey_Combined_Cleaned_Standardized_Names.csv in the same GitHub
...
# ------------------------------------------------------------
st.set_page_config(
    page_title="Pakistan LPG Combined Survey Dashboard",
    page_icon="🔥",
    layout="wide",
...
REQUESTED_WINDOWS_PATH = Path(r"C:\Ahmed Zahid Malik\Python\LPG Survey Dashboard\LPG_Survey_Combined_Cleaned_Standardized_Names.csv")
REQUESTED_WINDOWS_PATH_ALT = Path(r"C:\Ahmed Zahid Malik\Python\LPG Survey Dashboard\LPG_Survey_Combined_Cleaned_Standardized_Names (1).csv")
DEFAULT_PATHS = [
    REQUESTED_WINDOWS_PATH,
    REQUESTED_WINDOWS_PATH_ALT,
    Path(__file__).resolve().with_name("LPG_Survey_Combined_Cleaned_Standardized_Names.csv"),
    Path(__file__).resolve().with_name("LPG_Survey_Combined_Cleaned_Standardized_Names (1).csv"),
    Path(__file__).resolve().with_name("Clean-Data-lpg-survey.csv"),
    Path("/mnt/data/LPG_Survey_Combined_Cleaned_Standardized_Names.csv"),
    Path("/mnt/data/LPG_Survey_Combined_Cleaned_Standardized_Names (1).csv"),
...
def read_csv(uploaded_file=None) -> pd.DataFrame:
    ...
    if path is None:
        raise FileNotFoundError("LPG_Survey_Combined_Cleaned_Standardized_Names.csv not found. Upload it from the sidebar or place it beside this app file.")
...
st.sidebar.markdown(
    f'<div class="sidebar-note">Primary local path:<br><b>{REQUESTED_WINDOWS_PATH}</b><br><br>For online deployment, keep <b>LPG_Survey_Combined_Cleaned_Standardized_Names.csv</b> beside this app file. The older <b>Clean-Data-lpg-survey.csv</b> remains only as a fallback.</div>',
    unsafe_allow_html=True,
)
with st.sidebar.expander("Optional CSV upload", expanded=False):
    uploaded = st.file_uploader("Upload LPG_Survey_Combined_Cleaned_Standardized_Names.csv", type=["csv"])

try:
    df_all = prepare_data(uploaded)
except Exception as exc:
    st.error("Could not load LPG_Survey_Combined_Cleaned_Standardized_Names.csv. Upload the combined standardized file from the sidebar or place it beside the app.")
    st.exception(exc)
    st.stop()
...
st.markdown(
    f"""
    <div class="hero">
        <div class="hero-title">Pakistan LPG Country Demand Survey Dashboard - Combined Cleaned Dataset</div>
    </div>
    """,
    unsafe_allow_html=True,
)
...
st.markdown(
    '<div class="note-box"><b>Combined cleaned dataset note:</b> This dashboard uses <b>LPG_Survey_Combined_Cleaned_Standardized_Names.csv</b>, which combines the student/field responses and online LPG survey responses after column harmonization and name standardization. Use percentage-first visuals for comparison because sample sizes differ by segment.</div>',
    unsafe_allow_html=True,
)
...
st.sidebar.download_button(
    ...
    file_name="filtered_lpg_combined_survey_data.csv",
    mime="text/csv",
    use_container_width=True,
)
...
st.markdown(
    f"""
    <div class="note-box">
        <b>Run command:</b> <code>python -m streamlit run DashLPG_Combined_Cleaned_Names_Dashboard.py --server.address 0.0.0.0 --server.port 8501</code>
    </div>
    """,
    unsafe_allow_html=True,
)
